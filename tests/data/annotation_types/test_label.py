from labelbox.data.annotation_types.classification.classification import Classification, ClassificationAnswer, Radio, Subclass, Text
...
def test_schema_assignment_classification():
    ...
    radio_classification = Classification(value=Radio(
        answer=ClassificationAnswer(display_name=option_display_name)),)
    text_classification = Classification(value=Text(answer="some text"))
    label = Label(data=RasterData(arr=np.ones((32, 32, 3), dtype=np.uint8)),
                  annotations=[
                      Annotation(value=radio_classification,
                                 display_name=radio_display_name),
                      Annotation(value=text_classification,
...
def test_highly_nested():
    ...
    classification = Subclass(
        ...
        value=Radio(answer=ClassificationAnswer(
            display_name=option_display_name)),
        classifications=[
            Subclass(value=Radio(answer=ClassificationAnswer(
