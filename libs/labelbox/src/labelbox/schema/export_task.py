...
class FileRetrieverStrategy(ABC):  # pylint: disable=too-few-public-methods
    ...
    def _get_file_content(
        ...
        assert (
            len(response.content) == file_info.offsets.end - file_info.offsets.start + 1
        ), (
...
class FileRetrieverByOffset(FileRetrieverStrategy):  # pylint: disable=too-few-public-methods
    ...
    def _find_line_at_offset(self, file_content: str, target_offset: int) -> int:
    ...
    def get_next_chunk(self) -> Optional[Tuple[_MetadataFileInfo, str]]:
        ...
        file_content = file_content[self._current_offset - file_info.offsets.start :]
...
class Stream(Generic[OutputT]):
    ...
    def with_offset(self, offset: int) -> "Stream[OutputT]":
        ...
        self._reader.set_retrieval_strategy(FileRetrieverByOffset(self._ctx, offset))
        return self
    ...
    def start(self, stream_handler: Optional[Callable[[OutputT], None]] = None) -> None:
...
class ExportTask:
    ...
    def errors(self):
        ...
        BufferedStream(
            _TaskContext(
                self._task.client, self._task.uid, StreamType.ERRORS, metadata_header
            ),
