...
def test_export_v2_video(
    ...
    assert (
        export_data[0]["media_attributes"]["frame_rate"] == 10
    )  # as per the video_data fixture
    assert (
        export_data[0]["media_attributes"]["frame_count"] == 100
    )  # as per the video_data fixture
    ...
    assert (
        ...
        < export_frames[str(last_frame_id)]["objects"][first_exported_label_id][
            "bounding_box"
        ]["left"]
    )
