...
def test_above_annotation_limit_on_single_import_on_single_data_row(
    annotations_by_media_type,
):
    annotations_ndjson = list(
        itertools.chain.from_iterable(
            annotations_by_media_type[MediaType.Image]
        )
    )
    data_row_id = annotations_ndjson[0]["dataRow"]["id"]

    data_row_annotations = [
        annotation
        for annotation in annotations_ndjson
        if annotation["dataRow"]["id"] == data_row_id and "bbox" in annotation
    ]
...
def test_above_annotation_limit_divided_among_different_rows(
    ...
    with pytest.raises(ValueError):
        AnnotationImport._validate_data_rows(
            [first_data_row_annotation, second_data_row_annotation] * 2
        )
