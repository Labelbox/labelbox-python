...
def test_create_pdf(dataset):
    dataset.create_data_row(
        row_data={
            "pdfUrl": "https://lb-test-data.s3.us-west-1.amazonaws.com/document-samples/sample-document-1.pdf",
            "textLayerUrl": "https://lb-test-data.s3.us-west-1.amazonaws.com/document-samples/sample-document-custom-text-layer.json",
        }
    )
    dataset.create_data_row(
        row_data={
...
def test_create_descriptor_file(dataset):
    ...
    with mock.patch.object(
        client, "upload_data", wraps=client.upload_data
    ) as upload_data_spy:
        DescriptorFileCreator(client).create_one(
            items=[{"row_data": "some text..."}]
        )
        upload_data_spy.assert_called()
        call_args, call_kwargs = (
            upload_data_spy.call_args_list[0][0],
            upload_data_spy.call_args_list[0][1],
        )
