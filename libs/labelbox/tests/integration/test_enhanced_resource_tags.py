...
def test_resource_tags(client):
    ...
    for i, tag_type in enumerate(
        ...
        tag = EnhancedResourceTag.create(
            client, text=tag_text, color=tag_color, tag_type=tag_type
        )
        tags.append(tag)
...
def test_create_enhanced_resource_tag(client):
    ...
    # Create tag
    tag = EnhancedResourceTag.create(
        client, text=tag_text, color=tag_color, tag_type=ResourceTagType.Default
    )
...
def test_create_enhanced_resource_tag_without_type(client):
    ...
    tag = EnhancedResourceTag.create(client, text=tag_text, color=tag_color)
...
def test_search_by_text(client, test_resource_tags):
    ...
    type_filtered_results = EnhancedResourceTag.search_by_text(
        client, search_text="Test_Tag", tag_type=ResourceTagType.Default
    )
    ...
    non_existent_results = EnhancedResourceTag.search_by_text(
        client,
        search_text="NonExistentTag12345",
