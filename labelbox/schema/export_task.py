...
class FileRetrieverStrategy(ABC):  # pylint: disable=too-few-public-methods
    ...
    @abstractmethod
    def get_next_chunk(self) -> Tuple[_MetadataFileInfo, str]:
        """Retrieves the file."""
    ...
    def _get_file_content(
        ...
        assert len(
            ...
            f"expected {file_info.to_offset - file_info.from_offset + 1} bytes, "
...
class FileRetrieverByOffset(FileRetrieverStrategy):  # pylint: disable=too-few-public-methods
    ...
    def _find_line_at_offset(self, file_content: str, target_offset: int):
        stack = []
        line_number = 0
...
class Stream(Generic[OutputT]):
    ...
    def start(self, stream_handler: Callable[[OutputT], None] = None) -> None:
