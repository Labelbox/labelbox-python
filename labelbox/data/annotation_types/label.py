...
class Label(BaseModel):
    ...
    def create_data_row(self, dataset, signer):
        data_row = dataset.create_data_row(row_data = self.create_url_for_data(signer))
    ...
    def assign_schema_ids(self, ontology_builder):
        def assign_classification_schema_ids(annotation, tools):
            ...
            if annotation.schema_id is None:
                if tool is None:
                    raise ValueError(f"No classification matches display name {annotation.display_name}."
            ...
            if isinstance(annotation.value.answer, ClassificationAnswer):
                ...
                if not isinstance(annotation.value.answer[0], ClassificationAnswer):
            ...
            if answers is not None:
                for answer in answers:
                    ...
                    if option is None:
                        raise ValueError(f"No option matches display name {answer.display_name}."
                                        f"Must be one of {list(option.keys())}.")
                    answer.schema_id = option.feature_schema_id
                    subclass_options = {option.name: option for option in option.options}
                    for subclass in annotation.classifications:
                        assign_classification_schema_ids(subclass, subclass_options)

        for annotation in self.annotations:
            #tool = tools.get(annotation.display_name)
            #classification = classifications.get(annotation.display_name)
            if isinstance(annotation, Classification):
                assign_classification_schema_ids(annotation, {tool.name : tool for tool in ontology_builder.tools})
            elif isinstance(annotation, (Geometry, TextEntity)):
                tools = {tool.name : tool for tool in ontology_builder.tools}
                tool = tools.get(annotation.display_name)
                if annotation.schema_id is None:
                    if tool is None:
                        raise ValueError(f"No tool matches display name {annotation.display_name}."
                                     f"Must be one of {list(tools.keys())}.")
                    annotation.schema_id = tool.feature_schema_id
