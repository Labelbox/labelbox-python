...
class NDPoint(NDObject):
    ...
    def from_common(cls, annotation, data):
        return cls(
            point =  {'x' : annotation.value.x, 'y': annotation.value.y},
            dataRow = DataRow(id = data.uid),
            schemaId = annotation.schema_id,
            uuid = annotation.extra.get('uuid'),
            classifications = [NDSubclassification.from_common(annot) for annot in annotation.classifications]
        )
...
class NDLine(NDObject):
    ...
    def from_common(cls, annotation, data):
        return cls(
            line = [ {'x' : pt.x, 'y' : pt.y} for pt in annotation.value.points],
            dataRow = DataRow(id = data.uid),
            schemaId = annotation.schema_id,
            uuid = annotation.extra.get('uuid'),
            classifications = [NDSubclassification.from_common(annot) for annot in annotation.classifications]
...
class NDPolygon(NDObject):
    ...
    def from_common(cls, annotation, data):
        return cls(
            polygon = [ {'x' : pt.x, 'y' : pt.y} for pt in annotation.value.points],
            dataRow = DataRow(id = data.uid),
            schemaId = annotation.schema_id,
            uuid = annotation.extra.get('uuid'),
            classifications = [NDSubclassification.from_common(annot) for annot in annotation.classifications]
...
class NDRectangle(NDObject):
    ...
    def to_common(self):
        return Rectangle(
            ...
            end = Point(x = self.bbox.left + self.bbox.width, y = self.bbox.top + self.bbox.height)
        )
...
class NDMask(NDObject):
    ...
    def to_common(self):
        return Mask(
            mask = RasterData(url = self.mask.instanceURI),
    ...
    def from_common(cls, annotation, data):
        return cls(
            mask = _Mask(
                instanceURI = annotation.value.mask.url,
...
class NDObject:
    ...
    def to_common(annotation):
        ...
        classifications = [NDSubclassification.to_common(annot) for annot in annotation.classifications]
        return ObjectAnnotation(value =  common_annotation, schema_id = annotation.schemaId, classifications = classifications, extra = {'uuid' : annotation.uuid})
...
NDObjectType = Union[Bbox, NDLine, NDPolygon, NDPoint, NDRectangle,NDMask, NDTextEntity]
