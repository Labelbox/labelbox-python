...
def vector_to_coco_segment_info(canvas: np.ndarray, annotation: ObjectAnnotation,
...
def mask_to_coco_segment_info(canvas, annotation, category_id):
...
def process_label(label, idx, image_root, seg_root):
    ...
    mask_file = os.path.join(seg_root, mask_file)
    Image.fromarray(canvas.astype(np.uint8)).save(mask_file)
    return image, PanopticAnnotation(image_id=image_id,
...
class CocoPanopticDataset(BaseModel):
    ...
    def to_common(self, image_root, seg_root):
        ...
        for image in self.images:
            ...
            if category.isthing:
                # TODO: Convert masks into polygons here..
                pass
